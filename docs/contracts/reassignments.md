...
Fuente: [confirmación backend](../archive/backend/contrato-front-reasignaciones.md). Aplican las [reglas comunes](common.md). Activación y garantías pendientes de PR156 en [estado](../integration/status.md).
...
reassignment-workflow.service.spec.ts y http-reassignment.adapter.spec.ts cubren comportamiento local. El runner backend histórico de candidatos no certifica reasignación/cancelación completa. [Ejemplos sintéticos](examples/ejemplos-reasignaciones.json).
